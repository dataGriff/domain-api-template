...
describe('Auth routes', () => {
  describe('POST /v1/auth/register', () => {
    ...
    it('registers a new walker', async () => {
      const res = await request(app).post('/v1/auth/register').send({
    ...
    it('returns 409 for duplicate email', async () => {
      await request(app).post('/v1/auth/register').send({
        ...
        firstName: 'A', lastName: 'B', role: 'owner',
      });
      const res = await request(app).post('/v1/auth/register').send({
        email: 'dup@example.com', password: 'password123',
        firstName: 'A', lastName: 'B', role: 'owner',
  ...
  describe('POST /v1/auth/login', () => {
    it('logs in with correct credentials', async () => {
      await request(app).post('/v1/auth/register').send({
        ...
        firstName: 'L', lastName: 'U', role: 'owner',
      });
  ...
  describe('POST /v1/auth/logout', () => {
    it('returns 204 with valid token', async () => {
      const reg = await request(app).post('/v1/auth/register').send({
        ...
        firstName: 'L', lastName: 'O', role: 'owner',
      });
  ...
  describe('POST /v1/auth/refresh', () => {
    it('returns new tokens with valid refresh token', async () => {
      const reg = await request(app).post('/v1/auth/register').send({
        ...
        firstName: 'R', lastName: 'F', role: 'owner',
      });
